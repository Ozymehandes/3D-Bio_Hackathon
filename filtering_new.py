...
def combine_scores_and_rank(consensus_scores, embedding_scores, neural_net_scores, 
                          consensus_weight=0.4, embedding_weight=0.4, neural_net_weight=0.3):
...
def evaluate_accuracy(segment_sequences, consensus_scores, embedding_scores, neural_scores, ranked_segments, true_labels_dict, optimization_metric='f1'):
    ...
    fig_neural, _, neural_optimal_thresholds = plot_threshold_analysis(
        true_labels, neural_scores_arr, 'Neural Similarity Scoring',
...
def generate_comprehensive_plots(consensus_scores, embedding_scores, neural_scores, ranked_segments, 
                                segment_sequences, accuracy_results=None, save_prefix="project4"):
    """Generate all plots for Project 4 analysis."""
    
    # Extract data for plotting
    consensus_scores = [result['score'] for result in consensus_scores.values()]
    embedding_scores_list = list(embedding_scores.values())
    neural_scores_list = list(neural_scores.values())
    combined_scores = [score for _, score, _ in ranked_segments]
    ...
    # # Plot 2: Consensus vs Embedding vs neural scatter
    ...
    # # Plot 6: Score correlation
    # axes[1, 2].scatter(consensus_scores, combined_scores, alpha=0.6, color='red', label='Consensus vs Combined')
    # axes[1, 2].scatter(embedding_scores_list, combined_scores, alpha=0.6, color='blue', label='Embedding vs Combined')
    # axes[1, 2].set_xlabel('Individual Score')
    ...
    if accuracy_results:
        ...
        # Plot 9: Confusion matrix for best method
        best_method = max(method_names, key=lambda m: accuracy_results[m]['accuracy'])
        cm = confusion_matrix(accuracy_results['true_labels'], accuracy_results[best_method]['predictions'])
        
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[2, 2])
        axes[2, 2].set_title(f'Confusion Matrix - {best_method.title()}')
...
def run_comprehensive_project4_pipeline(
    ...
    neural_net_scores = pickle.load(open(f"data/network_scores.pkl", "rb"))
    neural_net_scores = {
    ...
    ranked_segments = combine_scores_and_rank(
        ...
        consensus_weight=consensus_weight, embedding_weight=embedding_weight, neural_net_weight=neural_net_weight
    )
    ...
    if true_labels_dict:
        ...
        accuracy_results = evaluate_accuracy(
            ...
            ranked_segments, true_labels_dict, optimization_metric='f1'
        )
